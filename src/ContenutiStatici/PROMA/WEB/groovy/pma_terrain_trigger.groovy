...
public class pma_terrain_trigger extends com.geowebframework.dataservice.querybuilder.EventTrigger {
...
def primo=services.queryService.executeQuery("select count(substring(cod_building,9,10)) as primo from pma_building where (substring(cod_building,3,6)=#{map.city} or substring(cod_building,3,3)=#{map.city}) and type_ownership='Terreno'",valuesMap)[0].primo;
...
if(primo==0 || primo==null || primo==''){
	...
	def prog=services.queryService.executeQuery("select max(substring(cod_building,9,10))+1 as prog from PMA_BUILDING where (substring(cod_building,3,6)=#{map.city} or substring(cod_building,3,3)=#{map.city}) and type_ownership='Terreno'",valuesMap)[0].prog;
	...
	*/
    ...
    public boolean beforeUpdate(HashMap<String,Object> valuesMap,HashMap<String,Object> OldvaluesMap){
		...
		if(OldvaluesMap.city!=valuesMap.city && valuesMap.city!=null){
			throw new RuntimeException('<b>Attenzione</b><br><br> Non e\' possibile cambiare il comune del terreno!');
		}
